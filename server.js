// ==================== FILE: server.js (GỘP TOÀN BỘ - ĐÃ SỬA LỖI LẤY SỐ DƯ & LỊCH SỬ) ====================
...
class Zalopay {
    ...
    // PHƯƠNG THỨC LẤY LỊCH SỬ MỚI - CHỈ LẤY TỪ API DANH SÁCH, KHÔNG GỌI CHI TIẾT TỪNG CÁI
        ...
        try {
            ...
            if (data && data.data && Array.isArray(data.data.transactions)) {
                const transactions = data.data.transactions.map(tx => {
                    // Xác định loại giao dịch dựa trên sign (1 = nhận, 0 = gửi)
                    ...
                    // Lấy lời nhắn từ template_info nếu có
                    let note = '';
                ...
                // Lọc chỉ lấy giao dịch thành công (status 1)
    ...
    try {
        ...
        switch (action) {
            case 'LOGIN': {
                ...
                // Kiểm tra cookie bằng cách gọi profile
            ...
            case 'history': {
                ...
                z.loadData(account);
                const history = await z.getTransactionsSimple(30); // lấy 30 giao dịch gần nhất
